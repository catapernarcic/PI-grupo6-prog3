...
export default class TarjetaP extends Component {
  ...
  agregarFav(id) {
    ...
    const existeFavorito = favoritos.filter(fav => fav.id === id && fav.tipo === 'movie').length > 0;
  ...
  sacaFav(id) {
    const favoritos = JSON.parse(localStorage.getItem('favoritos')) || [];
    const nuevosFavoritos = favoritos.filter(fav => !(fav.id === id && fav.tipo === 'movie'));
    localStorage.setItem('favoritos', JSON.stringify(nuevosFavoritos));

    this.setState({ esFav: false, favBoton: 'Agregar a favoritos' });
    
    if (this.props.onFavoritoEliminado) {
      this.props.onFavoritoEliminado(id, 'movie');
    }
  }
 ...
 componentDidMount(){
    ...
    const esFavorito = favoritos.filter(fav => fav.id === this.props.id && fav.tipo === 'movie').length > 0;
